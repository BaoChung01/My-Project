...
void clear_buffer(unsigned char screen_buffer[][132])
{
	int i,j;
	for (i=0;i<8;i++)
	{
...
void update_buffer(ImgType img, unsigned short img_num, unsigned char screen_buffer[][132])
{
int x_dir, y_dir, endx,endy, cnt;
	if((img.w+img.x_pos)>132)
	{
		endx = 131;
	}
	else
...
void update_str_buffer(short Ypos, short Xpos,char str[], unsigned char screen_buffer[][132])
{
...
void print_buffer(char i2c,unsigned char screen_buffer[][132])
{
	oled_pos(i2c,0,0);
...
void update_buffer_bit(ImgType img,unsigned short img_num,unsigned char screen_buffer[][132])
{
	int endx, startx,x_dir,endy, starty,y_dir,y_offset,cnt_1,cnt_2 ;
	int buffer_height = 8;
	int buffer_witdh = 132;
