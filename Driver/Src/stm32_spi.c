...
void SPI_Init(SPI_Handle_t *pSPIHandle)
{
 ...
 //2.Config the bus config
  ...
  {
    //BIDI mode shoukd be cleared
     tempreg&=~(1<<SPI_CR1_BIDIMODE);
     //RXONLY bit must be set
     tempreg |= (1<< SPI_CR1_RXONLY);
  }
 //3.Config the spi serial clock speed (Band Rate)
 ...
 //6.Config the CPHA
  ...
  tempreg|=pSPIHandle->SPIConfig.SPU_SSM << SPI_CR1_SSM;
...
uint8_t SPI_SendDataIT(SPI_Handle_t *pSPIHandle, uint8_t *pTxBuffer, uint32_t Len)
{
  ...
  //2. Mark the SPI state as busy in trasmission so that
...
static void spi_txe_interrupt_handle(SPI_Handle_t *pSPIHandle)
{
  if (pSPIHandle->pSPIx->CR1 &(1<<SPI_CR1_DFF))
      ...
      if(! pSPIHandle->TxLen)
      {
        //TxLen is zero, so close the spi trasmission and inform the application that
        // TX is over
